...
- **`NotesPanel`** — local `titleDraft` (commits debounced 500ms; dropped on doc switch so stale titles can't land on the new doc), tag dropdown, word count, save btn (`onManualSave(noteContent)`), download btn (`<title>.md` blob + persist same snapshot). Memo comparator covers `has_file/file` (open/download behavior).
